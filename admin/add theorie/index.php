...
<body>
    ...
    <div class="wall">
        ...
        <form action="" class="inputBox" method="post">
            ...
            <div>
                <textarea name="start" class="feedback" id="feedback" cols="30" rows="3" required
                    placeholder="مقدمة النظرية"></textarea>
            </div>
            <div>
                <textarea name="text" class="feedback" id="feedback" cols="30" rows="10" required
            ...
            <div>
                <label for="spoil">تحتوي على حرق</label>
                <input type="checkbox" name="spoil" id="spoil">
            </div>
            <div>
                <select name="type" id="">
                    <option value="theorie"> نظرية </option>
                    <option value="spoil"> تسريبات </option>
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['spoil'])) {
        $query = "INSERT INTO theorie (title, start, text, icon, type, spoil) VALUES ('$title','$start','$text','$icon','$type','1')";
        $result = $conn->query($query);
    } else {
        $query = "INSERT INTO theorie (title, start, text, icon, type, spoil) VALUES ('$title','$start','$text','$icon','$type','0')";
        $result = $conn->query($query);
    }
}
