<script>
    ...
    if (isset($_SESSION['login'])) {
        if ($result->num_rows < 1) {
            $result->fetch_assoc();
            $query = "UPDATE pirate SET fav = fav + 1 WHERE id = $id";
            $result = $conn->query($query);
            $query = "INSERT INTO fav (user_id, fav_id, type, react) VALUES ('$user_id','$id','$name','fav')";
            $result = $conn->query($query);

        } else {
            $query = "UPDATE pirate SET fav = fav - 1 WHERE id = $id";
            $result = $conn->query($query);
            $query = "DELETE FROM `fav` WHERE fav_id = '$id' AND react = 'fav' AND user_id = '$user_id' AND type = '$name'";
            $result = $conn->query($query);
        }
